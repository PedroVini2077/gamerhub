...
| `categoriaSaiuDaExperiencia.test.js` | **`[24/09]`** as duas pontas da remoção: um `DROP COLUMN category` numa migration (o dono escreveu em maiúsculas que não) e o seletor/badge voltando ao feed. Ignora **prosa** que cita o comando, senão o comentário que explica a decisão acusaria a si mesmo | `INV-CONTEUDO-007` |
