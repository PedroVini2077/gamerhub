...
| `docs/INVARIANTES.md` | **O que nunca pode acontecer** — regra permanente com ID estável, ligada ao achado que a originou e à trava que a protege | A história do achado (vai em `db/`), e regra sem trava |
| `docs/TRAVAS.md` | **O que impede** — os testes, roteiros e portões classificados por natureza, com o `INV-*` de cada um | A história de por que a trava nasceu (isso é `db/` e `SEGURANCA.md`) |
...
| `docs/regras/COTAS.md` | **`[24/09]`** O inventário do que cada serviço **conta**, o que acontece ao estourar, e se alguém fica sabendo. Mais as quatro regras de cota | Custo em dinheiro (não existe: tudo é plano gratuito) |
...
comentário HTML — `<!--n:src.arquivos-->415<!--/n-->` —, invisível no markdown
...
<!--n:docs.linhas-->25.702<!--/n--> linhas por precaução a cada sessão consome
