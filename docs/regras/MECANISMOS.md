...
| `scripts/edges-implantadas.mjs` (`npm run edges`) | **CI** (`implantar-edges.yml`), em `push` para `main` que toque `supabase/functions` — `[24/09]` esta coluna dizia *"ainda NÃO está no CI"*, e envelheceu | função **no ar** que não foi gerada deste código. Foi o buraco que deixou as duas correções da `send-email` mortas por 5 dias. Fora do CI de propósito: quase todas esperavam uma ação do dono (`BACKLOG.md`), e portão que grita por algo que ninguém pode resolver ensina a ignorar o canal |
...
| `rascunhoDeIaNaoDeriva.test.js` | `npm test`, **reprova** | `[25/09]` a IA que rascunha matéria voltando a poder o que ela não pode. Ela **lê a Edge Function** e exige quatro coisas: nenhum caminho de escrita no banco (o cliente de service role que ela usa para gritar em `admin_logs` passa por cima de toda a RLS e do corte editorial), a porta exigindo `is_staff()`, o mínimo de notas igual nos dois lados, e o marcador `[CONFERIR: …]` da instrução casando com quem o conta na tela. As quatro falham em **silêncio**, e a última é a pior: a tela diria "0 lacunas" sobre um rascunho cheio delas |
| `modeloDaIaEhDoNossoPlano.test.js` | `npm test`, **reprova** | `[26/09]` modelo de IA que a nossa conta nao alcanca. O `llama-3.3-70b-versatile` era modelo de PRODUCAO — eu conferi isso — e dava `HTTP 404` mesmo assim, porque e Enterprise. Pior: a evidencia passou pelas minhas maos, a tabela do plano gratis nao o listava, e eu li como "a pagina nao tem a informacao" em vez de "o modelo nao esta no plano" |
| `erroDoBancoNaoVazaCru.test.js` | `npm test`, **reprova** | `[26/09]` o texto do Postgres em inglês voltando para a tela. A parte que não depende de ninguém lembrar é a terceira checagem: ela **lê as migrations** e reprova a constraint nomeada que não tem frase em português — sem isso, regra nova só é descoberta quando alguém a esbarra, que foi exatamente o caso (`violates check constraint "news_articles_corpo_exigido_no_ar"` embaixo dos botões do painel). **Achou 6 constraints sem frase no 1º run** |
| `fonteEscrevePortugues.test.js` | `npm test`, **reprova** | `[26/09]` fonte que não sabe escrever português entrando no site. **É o exemplo mais limpo do que esta seção diz logo abaixo:** o defeito da Orbitron (o til virando crase no tamanho dos títulos) é PERCEPTUAL, e as quatro medições automáticas que eu tentei — glifo existe, glifo é próprio, é mais largo que a crase, bitmap difere — todas passavam. Então o mecanismo é o mais fraco que resolve, uma lista com data e método, e está declarado como tal em vez de disfarçado de verificação |
| `publicarTemCaminho.test.js` | `npm test`, **reprova** | `[26/09]` publicar virou ROTA, e rota sem porta e feature invisivel: ela continua respondendo, o build passa, nenhum teste de rota quebra — e ninguem consegue publicar. A trava exige as duas portas (a linha do feed e o botao da barra) e que a linha SUMA para quem nao pode publicar. **Ela mesma nasceu com o defeito que vigia**: procurava o nome do componente no arquivo inteiro, e a linha de `import` a fazia passar sobre um feed sem porta — descoberto reinjetando. **`[26/09]`** Ganhou a checagem de COLISAO DE NOME depois de o CI reprovar: o botao da barra se chamava "Publicar", igual ao que ENVIA no compositor. O `strict mode violation` era o sintoma barulhento; o silencioso era o `painel-admin.mjs` perder a checagem do corte editorial do News, que procura exatamente por esse nome |
| `radarDePautasNaoInventa.test.js` | `npm test`, **reprova** | `[26/09]` o radar de pautas citando fonte inventada. O modelo devolve URLs, e nada o impede de escrever uma plausível de um site conhecido — que, do lado de quem lê, é indistinguível de apuração. A trava exige o conjunto fechado dos endereços coletados, o descarte da pauta que perdeu todas as fontes, e que o descarte GRITE. **Achou um bug no parser de RSS no 1º run**. `[26/09]` Cobre tambem a `fatiaJusta`: a funcao mandava ao modelo os 60 primeiros itens NA ORDEM EM QUE OS FEEDS RESPONDIAM, entao fonte rapida comia a vaga de fonte boa — com 13 fontes para 60 vagas, uma fonte inteira podia nao chegar |
