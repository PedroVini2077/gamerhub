...
| **INV-EDIT-008** | **`[26/09]`** **O modelo que a Edge Function chama existe PARA A NOSSA CONTA.** Não basta ser modelo de produção: o `llama-3.3-70b-versatile` era, e devolvia `404` — é Enterprise. O Groq responde `404` e não `403` quando a conta não alcança, então nem a mensagem dizia "sem acesso". A pergunta é se ele está na tabela de limites **do plano que pagamos** | o dono recebeu `HTTP 404` no primeiro clique, 26/09 | `modeloDaIaEhDoNossoPlano.test.js` |
