...
| `e2e/` | Testes em navegador de verdade: rotas, fluxos autenticados, painel de admin, portas das Edge Functions, **portas do banco** (`portas-do-banco.mjs`, o único que fala com o Postgres), **portas da web** (`portas-da-web.mjs`, `[19/09]` — a borda HTTP: cabeçalho de segurança e vazamento de arquivo, o único que bate na PRODUÇÃO), **a CSP num navegador** (`politica-de-conteudo.mjs`, `[24/09]`), o laço da cena 3D, e **conteúdo visível** (`conteudo-visivel.mjs`, que rola as páginas públicas num tamanho de celular e reprova o que ficar em `opacity: 0`) |
