...
| `e2e/` | Testes em navegador de verdade: rotas, fluxos autenticados, painel de admin, portas das Edge Functions, **portas do banco** (`portas-do-banco.mjs`, o único que fala com o Postgres), **portas da web** (`portas-da-web.mjs`, `[19/09]` — a borda HTTP: cabeçalho de segurança por VALOR, **as seis diretivas travadas da CSP** (`[24/09]`) e vazamento de arquivo; o único que bate na PRODUÇÃO), **a CSP num navegador** (`politica-de-conteudo.mjs`, `[24/09]` — que ela não quebra a tela, enquanto o de cima confere que ela continua no ar), e **conteúdo visível** (`conteudo-visivel.mjs`, que rola as páginas públicas num tamanho de celular e reprova o que ficar em `opacity: 0`) |
