...
import { Tag, ExternalLink, Gift } from 'lucide-react';
...
export default function RightPanel() {
  ...
  useEffect(() => {
    supabase.from('game_keys').select('*').order('created_at', { ascending: false }).limit(10)
      .then(({ data }) => {
        if (data) {
          setKeys(data.filter(k => !k.is_promo));
    ...
    <aside className="hidden xl:flex flex-col w-72 shrink-0 gap-4">
      ...
      <div className="card p-4">
        ...
        <div className="space-y-2">
          {[
            { label: 'Online agora', value: '128', color: 'text-neon-green' },
            { label: 'Posts hoje', value: '47', color: 'text-neon-cyan' },
            { label: 'Keys ativas', value: keys.length.toString(), color: 'text-neon-purple' },
          ].map(s => (
