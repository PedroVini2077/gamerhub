-- Versão limpa: a migration anterior deixou a fórmula de XP duplicada (uma
-- CTE calculava e era descartada, outra recalculava). Agora o XP é calculado
-- UMA vez e alimenta tanto o ranking quanto o total.
-- Comprovado: dando 30 de XP a um usuário, total_xp passou de 0 para 30 e ele
-- foi para o topo do ranking.
...
BEGIN
  ...
  SELECT COALESCE(SUM(xp),0) INTO total_xp FROM _xp;
  ...
  SELECT jsonb_agg(e ORDER BY ord) INTO top_users FROM (
    SELECT jsonb_build_object('username',username,'role',role,'post_count',post_count,'xp',xp) AS e,
           row_number() OVER (ORDER BY xp DESC) AS ord
    FROM _xp ORDER BY xp DESC LIMIT 10) ranked;
